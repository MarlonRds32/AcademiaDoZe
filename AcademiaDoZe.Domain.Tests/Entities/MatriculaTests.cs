...
public class MatriculaTests
{
    ...
    public void Deve_FalharOuPassar_Criacao_Quando_ValorDoPlano(int planoValue)
    {
        ...
        else
        {
            ...
            Assert.Equal(aluno.Id, result.Value!.AlunoMatricula.Id);
        }
    }
    ...
    [Fact(DisplayName = "Matricula: menor de 16 anos com laudo -> sucesso")]
    public void Deve_Criar_Com_Sucesso_Quando_Menor16ComLaudo()
    {
        var aluno = Aluno.Criar(1, "João", "529.982.247-25", DateOnly.FromDateTime(DateTime.Today.AddYears(-15)),
            "(11) 91234-5678", "user@example.com",
            Logradouro.Criar(1, "12345-678", "Rua", "Bairro", "Cidade", "SP", "Brasil").Value!,
            "123", "", "Abcdef", Arquivo.Criar(new byte[] { 1 }).Value!).Value!;
        var laudo = Arquivo.Criar(new byte[] { 1, 2, 3 }).Value!;

        var result = Matricula.Criar(1, aluno, MatriculaPlano.Mensal, DateOnly.FromDateTime(DateTime.Today),
            "Objetivo", MatriculaRestricoes.None, laudo);
