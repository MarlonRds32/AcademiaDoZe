...
public class ValueObjectsTests
{
...

    {
        ...
        Assert
        .Equal
        (logradouro.Id, result.Value!.Logradouro.Id);
    ...
    [Theory(DisplayName = "Cep: contém letras -> CEP_DIGITOS")]
    [InlineData("abcde-123")]
    [InlineData("12345-abc")]
    public void Deve_Falhar_Criacao_Quando_CepContemLetras(string input)
    {
        // Arrange + Act: chama Cep.Criar com um CEP que tem letras misturadas
        var result = Cep.Criar(input);

        // Assert: espera falha, pois depois de limpar os dígitos, sobra menos de 8 números
        Assert.True(result.IsFailure);
        Assert.Contains(result.Notifications, n => n.Mensagem == "CEP_DIGITOS");
    }


    [Fact(DisplayName = "Endereco: complemento vazio é aceito (opcional)")]
    public void Deve_Criar_Endereco_Quando_ComplementoVazio()
    {
        // Arrange
        var logradouro = Logradouro.Criar(1, "12345-678", "Rua Teste", "Bairro", "Cidade", "SP", "Brasil").Value!;

        // Act
        var result = Endereco.Criar(logradouro, "100", "");

        // Assert
        Assert.True(result.IsSuccess);
        Assert.Equal("", result.Value!.Complemento);
    }

    [Theory(DisplayName = "Telefone: fixo (10 dígitos) -> TELEFONE_DIGITOS")]
    [InlineData("(11) 3456-7890")]
    [InlineData("1134567890")]
    public void Deve_Falhar_Criacao_Quando_TelefoneFixoComDezDigitos(string input)
    {
        // Arrange + Act
        var result = Telefone.Criar(input);

        // Assert: seu Telefone.Criar exige exatamente 11 dígitos, então fixo (10) falha
        Assert.True(result.IsFailure);
        Assert.Contains(result.Notifications, n => n.Mensagem == "TELEFONE_DIGITOS");
    }
    [Theory(DisplayName = "Email: formatos inválidos diversos -> EMAIL_FORMATO")]
    ...
    [Fact(DisplayName = "Email: formato válido com subdominio")]
    public void Deve_Criar_Email_Quando_ValidoComSubdominio()
    {
        var result = Email.Criar("usuario@mail.example.com");

        Assert.True(result.IsSuccess);
        Assert.Equal("usuario@mail.example.com", result.Value!.Valor);
    }
    ...
    [Fact(DisplayName = "Arquivo: array vazio é aceito (0 bytes não excede limite)")]
    public void Deve_Criar_Arquivo_Quando_ArrayVazio()
    {
