...
public class ColaboradorInfrastructureTests : TestBase
{
    ...
    internal static async Task<Colaborador> CriarEInserirColaboradorAsync(ColaboradorRepository colaboradorRepo, LogradouroRepository logradouroRepo)
    {
        ...
        var colaboradorResult = Colaborador.Criar(
            id: 0,
            nome: "Marlon" + Guid.NewGuid().ToString("N")[..5],
            cpf: GerarCpf(),
    ...
    public async Task Colaborador_Atualizar_Sucesso()
    {
        ...
        var colaboradorOriginalResult = Colaborador.Criar(
            0, "Marlon", GerarCpf(), new DateOnly(1995, 5, 15),
            GerarTelefone(), GerarEmail(), logradouro, "200", "Rodrigues",
            "SqlServer1", foto, new DateOnly(2023, 1, 1),
            ColaboradorTipo.Instrutor, ColaboradorVinculo.CLT);

        Assert.True(colaboradorOriginalResult.IsSuccess, colaboradorOriginalResult.IsFailure
            ? string.Join(", ", colaboradorOriginalResult.Notifications.Select(n => n.Mensagem))
            : "");

        var colaborador = await _colaboradorRepo.Adicionar(colaboradorOriginalResult.Value!);

        var novoNome = "Marlon Editado";
        var colaboradorAtualizadoResult = Colaborador.Criar(
            id: colaborador.Id,
            nome: novoNome,
        ...
        Assert.True(colaboradorAtualizadoResult.IsSuccess, colaboradorAtualizadoResult.IsFailure
            ? string.Join(", ", colaboradorAtualizadoResult.Notifications.Select(n => n.Mensagem))
            : "");

        var colaboradorAtualizado = colaboradorAtualizadoResult.Value!;
    ...
    public async Task Colaborador_Atualizar_LancaExcecaoQuandoInexistente()
    {
        ...
        var colaboradorInexistenteResult = Colaborador.Criar(
            id: 999999,
            nome: "Marlon",
            cpf: GerarCpf(),
        ...
        Assert.True(colaboradorInexistenteResult.IsSuccess, colaboradorInexistenteResult.IsFailure
            ? string.Join(", ", colaboradorInexistenteResult.Notifications.Select(n => n.Mensagem))
            : "");

        var colaboradorInexistente = colaboradorInexistenteResult.Value!;
    ...
    public async Task Colaborador_ObterPorVinculo_FiltragemCorreta()
    {
        ...
        {
            var colaborador = await CriarEInserirColaboradorAsync(_colaboradorRepo, _logradouroRepo);
            var senhaTexto = "NovaSenhaColaborador123";
            var novaSenhaResult = Senha.Criar(senhaTexto);

            Assert.True(novaSenhaResult.IsSuccess, novaSenhaResult.IsFailure
                ? string.Join(", ", novaSenhaResult.Notifications.Select(n => n.Mensagem))
                : "");

            var novaSenha = novaSenhaResult.Value!;
