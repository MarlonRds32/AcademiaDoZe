...
public class LogradouroInfrastructureTests : TestBase
{
    ...
    {
        var cep = GerarCep();
        var logradouroResult = Logradouro.Criar(0, cep, "Marlon_1", "Rodrigues_1", "MySql", "SC", "Brasil");
    ...
    {
        var cep = GerarCep();
        var logradouro = Logradouro.Criar(0, cep, "Marlon_2", "Rodrigues_2", "MySql", "SC", "Brasil").Value!;
        ...
        Assert.Equal("Marlon_2", inserido.Nome);
    ...
    public async Task Logradouro_Atualizar_Sucesso()
    {
        ...
        var logradouroAtualizado = Logradouro.Criar(logradouro.Id, novoCep, "Marlon_3", "Rodrigues_3", "MySql", "SC", "Brasil").Value!;
        var resultado = await _repository.Atualizar(logradouroAtualizado);
        Assert.NotNull(resultado);
        Assert.Equal("Marlon_3", resultado.Nome);
        Assert.Equal("Rodrigues_3", resultado.Bairro);
        Assert.Equal("MySql", resultado.Cidade);
        var noBanco = await _repository.ObterPorId(logradouro.Id);
        Assert.NotNull(noBanco);
        Assert.Equal("Marlon_3", noBanco.Nome);
    }
    [Fact]
    ...
    public async Task Logradouro_ObterPorCidade_FiltragemCorreta()
    {
        ...
        var logradouro = Logradouro.Criar(0, cep, "Marlon", "Rodrigues", cidadeUnica, "SC", "Brasil").Value!;
    ...
    public async Task Logradouro_ObterPorBairro_FiltragemCorreta()
    {
        ...
        var logradouro = Logradouro.Criar(0, cep, "Marlon", bairro, cidade, "SC", "Brasil").Value!;
        await _repository.Adicionar(logradouro);
