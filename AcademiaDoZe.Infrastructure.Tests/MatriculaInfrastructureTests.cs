...
public class MatriculaInfrastructureTests : TestBase
{
    ...
    public async Task Matricula_Adicionar_E_ObterPorId_Sucesso()
    {
        ...
        var obsRestricao = $"Usar medicação ao acordar - {DatabaseType}";
    ...
    public async Task Matricula_RestricoesMedicas_ComVariacoesMultiplaEscolha_PersisteEObtemCorretamente()
    {
        ...
        var obsRestricao = $"Evitar exercícios de alto impacto e hipertensão - {DatabaseType}";
    ...
    public async Task Matricula_Atualizar_LancaExcecaoQuandoInexistente()
    {
        ...
        var matriculaInexistenteResult = Matricula.Criar(
            id: 999999,
            alunoMatricula: aluno,
            plano: MatriculaPlano.Mensal,
            ...
            laudoMedico: null, 
            observacoesRestricoes: DatabaseType.ToString()
        );

        Assert.True(matriculaInexistenteResult.IsSuccess, matriculaInexistenteResult.IsFailure
            ? string.Join(", ", matriculaInexistenteResult.Notifications.Select(n => n.Mensagem))
            : "");

        var matriculaInexistente = matriculaInexistenteResult.Value!;
    ...
    public async Task Matricula_Atualizar_Sucesso()
    {
        ...
        var obsRestricaoAtualizada = $"Restrição médica atualizada com novas opções - {DatabaseType}";

        var matriculaAtualizadaResult = Matricula.Criar(
            id: inserida.Id,
            alunoMatricula: aluno,
        ...
        Assert.True(matriculaAtualizadaResult.IsSuccess, matriculaAtualizadaResult.IsFailure
            ? string.Join(", ", matriculaAtualizadaResult.Notifications.Select(n => n.Mensagem))
            : "");

        var matriculaAtualizada = matriculaAtualizadaResult.Value!;

        var resultado = await _matriculaRepo.Atualizar(matriculaAtualizada);
        Assert.NotNull(resultado);
        Assert.Equal(MatriculaPlano.Anual, resultado.Plano);
        Assert.Equal("Marlon", resultado.Objetivo);
        ...
        Assert.Equal("Marlon", noBanco.Objetivo);
    ...
    public async Task Matricula_ObterPorPlano_FiltragemCorreta()
    {
        ...
        Assert.Contains(trimestrais, m => m.AlunoMatricula.Id == aluno.Id && m.Plano == MatriculaPlano.Trimestral);
    }
}